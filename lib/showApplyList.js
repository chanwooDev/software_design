module.exports={
    ...
    showList:function(name,apply){
        ...
        if(apply == "가입")
            mention = mention + `<button class ="btn btn-secondary" onclick="location.href='/join_success/?name=${name}'">승낙</button>`;
        else if(apply == "탈퇴")
            mention = mention + `<button class ="btn btn-secondary" onclick="location.href='/withdraw_success/?name=${name}'">승낙</button>`;
